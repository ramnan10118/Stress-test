// Iteration 10: Home dashboard — full entrance animations with reduced motion support
...
const SERVICES = [
  { icon: ShieldCheck, label: "Zero Depreciation" },
  { icon: Wrench, label: "Roadside Assistance" },
];
...
export default function Prompt110() {
  ...
  const visible = reducedMotion || mounted;
  ...
  const fadeUp = (delay = 0): React.CSSProperties =>
    reducedMotion
      ...
      : {
          ...
          transition: `opacity var(--duration-slow) var(--ease-out-cubic) ${delay}ms, transform var(--duration-slow) var(--ease-out-cubic) ${delay}ms`,
        };
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
                {SERVICES.map(({ icon: Icon, label }, i) => (
                  <div key={label}>
                    ...
                    {i < SERVICES.length - 1 && <div style={{ marginTop: "var(--space-3)" }}><Separator /></div>}
                  </div>
                ))}
              </div>
            </CardContent>
      ...
      <nav
        ...
        style={{
          ...
          ...(reducedMotion ? {} : {
            ...
            transition: `transform var(--duration-moderate) var(--ease-out-cubic) 300ms`,
          }),
        }}
      >
