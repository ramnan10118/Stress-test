import { Bell, Car, ChevronRight, FileText, Phone, Home, User, Shield } from "lucide-react";
...
export default function Prompt071() {
  return (
      ...
      <main style={{ flex: 1, padding: "var(--space-6) var(--space-4)", paddingBottom: "calc(var(--scale-64) + var(--space-6))", maxWidth: 480, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
        ...
        <div>
          ...
          <Typography variant="body-sm" color="muted">
            Your policies are active and up to date.
          </Typography>
        </div>

        {/* Promotional banner */}
        <Alert variant="info" title="Refer a friend, earn ₹500">
          Share your referral code with friends. When they buy a policy, you both earn ₹500 cashback.
        </Alert>
            ...
            <CardContent>
              <div style={{ display: "flex", alignItems: "flex-start", gap: "var(--space-3)" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Typography variant="caption" color="muted">Expires 18 Apr 2026</Typography>
                </div>
      ...
      <nav aria-label="Main navigation" style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--grey-white)", borderTop: "1px solid var(--color-border-subtle)", display: "flex", paddingBottom: "env(safe-area-inset-bottom)", zIndex: "var(--z-sticky)" }}>
        {NAV_ITEMS.map(({ icon: Icon, label, active }) => (
          <button
            ...
            style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "var(--space-1)", background: "none", border: "none", cursor: "pointer", touchAction: "manipulation", padding: "var(--space-2) var(--space-2) var(--space-3)", color: active ? "var(--color-primary)" : "var(--color-text-muted)" }}
          >
            <Icon size={20} aria-hidden="true" />
