...
import { ChevronLeft, Check, X, ChevronDown, ChevronUp, TrendingUp, Info, Shield } from "lucide-react";
...
const PLANS: Plan[] = [
  { id: "tp", name: "Third Party", subtext: "Mandatory by law", premium: 2094, marketPremium: 2094, recommended: false, popular: false, accentColor: "var(--color-text-muted)", inclusions: ["Third party liability", "Personal accident cover", "Legal liability", "No own damage cover"] },
  { id: "comp", name: "Comprehensive", subtext: "Best for everyday drivers", premium: 5782, marketPremium: 8200, recommended: true, popular: true, accentColor: "var(--color-primary)", inclusions: ["Own damage + third party", "Personal accident cover", "Cashless repairs at 3,000+ garages", "24×7 roadside assistance"] },
  { id: "total", name: "ACKO Total Protect", subtext: "Maximum peace of mind", premium: 7499, marketPremium: 11500, recommended: false, popular: false, accentColor: "var(--color-success)", inclusions: ["Everything in Comprehensive", "Zero depreciation cover", "Engine protection", "Return to invoice"] },
];
...
// Iteration 10 (final): +subtext per plan, trust strip at bottom, consistent spacing audit
export default function Prompt215() {
  ...
  const getSavings = (plan: Plan) => {
    const diff = plan.marketPremium - plan.premium;
    ...
    <div style={{ minHeight: "100vh", background: "var(--color-surface)", display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ flex: 1, padding: "var(--space-5)", paddingBottom: "var(--space-20)", maxWidth: 480, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
              ...
              return (
                <div key={plan.id} role="listitem">
                  <Card variant="default" padding="md" style={{ position: "relative", overflow: "hidden", ...(plan.recommended ? { borderColor: "var(--color-primary)", borderWidth: 2 } : {}) }}>
                    ...
                    {/* ADDED: Subtext per plan */}
                    <Typography variant="body-sm" color="muted" style={{ display: "block", marginBottom: "var(--space-3)" }}>{plan.subtext}</Typography>

                    <div style={{ display: "flex", alignItems: "center", gap: "var(--space-1)" }}>
                      <Typography variant="caption" color="muted">Starting from</Typography>
                      <Tooltip content="Final premium may vary based on add-ons and discounts" side="top">
                        <span style={{ display: "inline-flex", cursor: "help", color: "var(--color-text-muted)" }}>
                          <Info size={12} aria-label="Price disclaimer" />
                        </span>
                      </Tooltip>
                    </div>

                    <Typography variant="display-lg" weight="bold" color="strong" style={{ display: "block", fontVariantNumeric: "tabular-nums" }}>
                      ₹{plan.premium.toLocaleString("en-IN")}<Typography variant="body-md" color="muted">/year</Typography>
                    </Typography>

                    {savings > 0 && (
                      <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", marginTop: "var(--space-1)", marginBottom: "var(--space-4)" }}>
                        <Typography variant="body-sm" color="muted" style={{ textDecoration: "line-through", fontVariantNumeric: "tabular-nums" }}>
                          ₹{plan.marketPremium.toLocaleString("en-IN")}
                        </Typography>
                        <Badge color="green" size="sm">Save ₹{savings.toLocaleString("en-IN")}</Badge>
                      </div>
                    )}
                    {savings === 0 && <div style={{ marginBottom: "var(--space-4)" }} />}
                  ...
                  {PLANS.map((p) => {
                    ...
                    return (
                      <TableHead key={p.id} style={{ textAlign: "center", minWidth: 120 }}>
                        ...
                        {savings > 0 && (
                          <div style={{ marginTop: "var(--space-1)" }}><Badge color="green" size="sm">Save ₹{savings.toLocaleString("en-IN")}</Badge></div>
                        )}
                      </TableHead>
                    );
                  })}
                  ...
                  <TableRow key={feat.name}>
                    ...
                    {[feat.tp, feat.comp, feat.total].map((included, i) => (
                      <TableCell key={i} style={{ textAlign: "center" }}>
                        {included ? <Check size={16} strokeWidth={2.5} style={{ color: "var(--color-success)" }} aria-label="Included" /> : <X size={16} strokeWidth={2} style={{ color: "var(--color-text-muted)" }} aria-label="Not included" />}
                      </TableCell>
                    ))}
                  </TableRow>
                ))}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "var(--space-4)", flexWrap: "wrap" }}>
          <Shield size={20} style={{ color: "var(--color-primary)", flexShrink: 0 }} aria-hidden="true" />
          {TRUST_ITEMS.map((item, i) => (
            <div key={item} style={{ display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
              {i > 0 && <span aria-hidden="true" style={{ color: "var(--color-border-subtle)" }}>·</span>}
              <Typography variant="caption" weight="medium" color="muted">{item}</Typography>
            </div>
          ))}
        </div>
      </main>
